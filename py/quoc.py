...
#                                                                             #
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
"""LLVM-based Quo compiler.

For debugging, run this module to print out the LLVM IR for Quo modules.  It
will read from files supplied on the command line, or stdin if no files are
specified.
"""
...
class CompilerVisitor(quo_ast.Visitor):
  """AST visitor that converts an AST to LLVM IR.

  Each visitor method returns an LLVM IR node.
  """
  ...
  def visit_module(self, node, args):
    ...
    # return '\n\n'.join(members)
    module = llvmlite.ir.Module(name='temp_module')
    return module
...
def quo_to_ir(quo_str):
  """Compiles a Quo module to LLVM IR."""
  ...
  return ast.accept(compiler_visitor)
...
if __name__ == '__main__':
  ...
  module = quo_to_ir(''.join(fileinput.input()))
  print(module)
